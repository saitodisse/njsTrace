...
Tracer.prototype.onEntry = function(args) {
	...
	this.onEntryArgs.sid       = sid;
	this.onEntryArgs.direction = 'in';
	this.onEntryArgs.date      = Date.now();
	...
	this.all_results.push(clone(this.onEntryArgs));
...
Tracer.prototype.onExit = function(args) {
	...
	// We are recycling the same args object again and again to ease on GC
	// This means that the formatters should NOT keep a reference to this object in some async callback as it will change
	this.onExitArgs.sid       = args.entryData.stackId;
	this.onExitArgs.direction = 'out';
	this.onExitArgs.date      = Date.now();
	...
	this.all_results.push(clone(this.onExitArgs));
 ...
 * save file to disk
 */
Tracer.prototype.save = function(filename) {
	var fs = require('fs');
	fs.writeFileSync(filename, JSON.stringify(this.all_results));
	console.log('# saved', this.all_results.length, 'timeline steps.');
};
